...
第一版代码已实现 `admission-v1`、`event-match-v1`、`confirmation-v1`、
`event-metrics-v1` 和 `event-lifecycle-v1` 的确定性规则。语义相似度采用 Sentence Transformers
生成的归一化 embedding，并由 pgvector 做租户内、时间窗内候选检索；最终 `ATTACH`、准入、
热度、风险与状态转换仍由上述规则裁决。

低可信但高潜在影响的信息不会直接成为已确认事件：即使综合准入分达到阈值，来源质量未达到
直接创建门槛时仍进入 `candidate`，等待独立来源数、最高来源质量和传播速度满足确认规则。
缺失互动基线、权威来源或平台覆盖时，对应因子保持 `null`，计算结果同时保存完整度，不以 0
冒充已观测数据。

尚未实现的是 Event Extractor 的真实模型调用、历史消息导入、后台调度与数据库写入服务；
因此当前状态是“规则与持久化结构已实现、运行入口尚未接线”，不是完整自动事件发现能力。
